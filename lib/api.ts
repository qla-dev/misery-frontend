...
export interface ApiGame { id: number; code: string; owner_id: number; started: boolean; host_in_lobby: boolean; lobby_member_ids: number[]; is_private: boolean; terminated_at: string | null; termination_reason: 'host_left' | 'host_inactive' | string | null; stack_id: number | null; stack: 'normal' | 'spicy' | string | null; target_score: number; winner_id: number | null; current_player_id: number | null; turn_owner_id: number | null; awaiting_finish: boolean; is_steal_turn: boolean; sync_driver: 'polling' | 'pusher' | 'ably' | 'reverb'; ingame_polling_interval_ms: number; pusher: { key: string; cluster: string; channel: string; event: string; heartbeat_interval_ms: number } | null; ably: { channel: string; event: string; token_endpoint: string; heartbeat_interval_ms: number } | null; reverb: { key: string; host: string; port: number; scheme: 'http' | 'https' | string; channel: string; event: string; heartbeat_interval_ms: number } | null; members: ApiUser[]; hands: Record<string, ApiCard[]>; current_card: ApiCard | null; moves: ApiMove[]; chat_messages: ApiChatMessage[] }
